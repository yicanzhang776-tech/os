...
| 角色 | 主要工作 | 负责人 |
|---|---|---|
| 项目负责人 | 赛题理解、路线确认、提交把关 | 李浩明 |
| 内核实现 | P0、Trap、内存、调度、用户态、文件系统 | 李浩明 |
| 教学设计 | Lab1-Lab7 任务拆分、提示和教师指南 | 李昊然、张义灿 |
| 测试与验收 | QEMU 脚本、主机单元测试、分阶段测试 | 李昊然、张义灿 |
| 文档与答辩 | 设计文档、演示脚本、PPT 和视频 | 李昊然、张义灿 |
...
同学可以先切换到需要学习的实验分支，再运行 `scripts/run-interactive-demo.ps1`。浏览器打开页面后，可查看当前分支和实验角色，点击运行按钮触发构建与 QEMU 执行，并结合实时串口证据理解代码行为。由于 P0 和 Lab1-Lab7 的 starter/solution 分支都包含同一版本的页面与桥接器，因此每个实验分支都能独立启动展示。

### 11.5 验证情况

我们补充了协议解析测试和本地桥接服务集成测试，覆盖 16 种仓库分支上下文、starter/solution 识别、TODO 与完成标志区分、串口事件解析、HTML 页面服务和 WebSocket 实时传输。最近一次验证中，相关 8 项自动测试全部通过。通过这些检查，可视化页面能够在不改变各实验核心代码和教学目标的前提下，为学生提供可重复、可追踪的学习反馈。
