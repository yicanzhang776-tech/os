...
可视化页面内的实验教学评价收集学习者对讲解、任务、提示、可视化和运行反馈的主观感受。题目会随 P0、Lab 和 starter/solution 变化，支持本机草稿、离线导出以及由使用者主动发送到项目负责人提供的 HTTPS 反馈地址；它不计算成绩或排名。
...
两类工具都不依赖云数据库、统一登录或 AI 自动评分。教学评价的轻量接收端只使用 Node.js 内置模块和本机 JSONL；教师评分工具仍保持纯本地。导出内容可能包含提交标识、评价和自由文本，公开分享前必须由教师人工匿名化和脱敏。
...
我们补充了教学评价、协议解析、运行记录和本地桥接服务测试，覆盖 17 种仓库分支上下文、starter/solution 识别、TODO 与完成标志区分、事件协议校验、时间线保存、导入导出与对比、HTML 页面服务和 WebSocket 实时传输。最新一次完整验证中，相关 111 项自动测试全部通过，并完成 JavaScript 与 Linux Shell 语法检查。通过这些检查，可视化页面能够在不改变各实验核心代码和教学目标的前提下，为学生提供可重复、可追踪的学习反馈。
...
安全边界也在目录层统一处理。只有经过检查的仓库相对路径才能生成源码链接，绝对路径、父目录跳转和网络共享路径会被拒绝。未登记或旧格式事件保留经过长度限制的原始信息，显示降级提示，但不推进知识地图，也不会造成页面异常。事件解释、回放和分支映射均在本机完成，不连接外部服务器，不上传代码、事件或终端日志。新增 Node 内置测试验证已知事件映射、未知事件降级、源码路径边界、17 种分支上下文不变，以及实时事件与回放事件解释一致。

本次补充完成后，可视化目录的完整 Node 内置测试共 111 项，新增测试与原有协议、反馈、运行记录、预测、状态模型和本地桥接器测试一起执行。事件解释只接受明确的 `os-demo.event/v1` 证据；保存记录会深度校验并重新计算预测对照，双方均无证据的状态也会单独列为“双方证据不足”，避免把未知误写成一致。
...
为了在不引入额外服务的前提下帮助学生排查实验现象，我们增加了独立的 `diagnostics.js`。模块只在本地读取当前 Lab、分支角色、构建结果、`os-demo.event/v1` 结构化事件、经过净化和长度限制的稳定输出以及最终运行状态，不调用 AI 模型、智能体、网络 API 或外部服务，也不改变现有事件协议。
...
8890 只提供健康检查、CORS 预检和评价提交接口。它限制请求为 32 KiB，按明确来源白名单处理 CORS，重新验证并净化记录，对同一来源进行每分钟 10 次的简单限流。`feedback.id` 是幂等键：新 ID 追加一行 JSON；相同 ID 和内容返回原回执；相同 ID 但内容不同返回 409，绝不覆盖原记录。写入失败不会返回成功。端口不提供管理页面。

8891 只绑定 `127.0.0.1` 并拒绝外部 Host，用同一 JSONL 生成 Lab、starter/solution 和使用者身份筛选结果、五道题平均值与分布、文字建议以及 JSON/CSV/Markdown 导出。浏览器渲染评价时只使用 `textContent`，服务端同时设置本机资源 CSP。课堂异地测试只把 8890 放入临时 HTTPS 隧道，8888、8891、SSH 和 Git 均不公开。
