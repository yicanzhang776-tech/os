...
演示模式的安全边界与实验模式保持一致。Ubuntu/Linux 使用 `sh scripts/run-interactive-demo.sh` 启动 4173 端口，演示时不添加 `--run`；页面不会因进入演示模式或选择推荐 Lab 而自动运行 QEMU、切换 Git 分支或上传代码和运行证据。演示专用 session 只记录本次会话的视图位置，一键重置不会删除已有运行记录、学生预测和教学反馈，真实运行仍必须由使用者明确确认。
