...
两类工具都不依赖服务器、数据库、统一登录或 AI 自动评分。导出内容可能包含姓名、提交标识、成绩和自由文本，公开分享前必须由教师人工匿名化和脱敏。
...
我们补充了教学评价、协议解析、运行记录和本地桥接服务测试，覆盖 17 种仓库分支上下文、starter/solution 识别、TODO 与完成标志区分、事件协议校验、时间线保存、导入导出与对比、HTML 页面服务和 WebSocket 实时传输。2026 年 8 月 9 日在本轮基线上执行完整测试，共 133 项自动测试全部通过；JavaScript 与 Linux Shell 语法检查仍以最终交付验收记录为准。通过这些检查，可视化页面能够在不改变各实验核心代码和教学目标的前提下，为学生提供可重复、可追踪的学习反馈。
...
本次补充完成后，可视化与教师评分工具的完整 Node 内置测试共 133 项，新增测试与原有协议、反馈、运行记录、预测、状态模型、本地桥接器和评分记录测试一起执行。事件解释只接受明确的 `os-demo.event/v1` 证据；保存记录会深度校验并重新计算预测对照，双方均无证据的状态也会单独列为“双方证据不足”，避免把未知误写成一致。
