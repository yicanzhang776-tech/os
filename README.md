...
该方案适合大学课程的小规模测试：项目负责人的 Ubuntu 虚拟机必须在线，Quick Tunnel 地址会变化，邀请码不等于完整账号认证，JSONL 不适合大量并发，并且暂时没有复杂班级管理、长期域名和高级统计。网络失败时仍可保存本机草稿或离线导出；后续可在有维护条件时再增加固定域名、正式 Tunnel、SQLite、备份恢复、匿名标识和正式权限管理。
