...
> 智能体使用：可结合任务状态、yield 和上下文切换事件提出验证步骤，但不能读取 solution/教师文件；回答不能替代 TCB、寄存器布局和调度不变量审查。

> 验收约定：学生按 `-Stage 1/2/3` 逐步验证；原始 starter 才使用 `-ExpectIncomplete`。Demo 运行证据可导出给教师评分工具，但不能代替上下文布局和调度状态审查。
