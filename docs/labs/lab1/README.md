...
推荐学习闭环：先在可视化页面保存预测，再运行当前分支；完成每项任务后执行对应 `-Stage`，最后回放真实事件并填写教学反馈。反馈不计分。
...
- [SOLUTION.md](SOLUTION.md)：参考实现说明，仅在 `lab1-solution` 分支提供。
- [TEACHER_GUIDE.md](TEACHER_GUIDE.md)：教师授课和验收建议，仅在 `lab1-solution` 分支提供。
