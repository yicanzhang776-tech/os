...
> 智能体使用：可结合地址转换、分配、释放和复用事件提出验证步骤，但不能读取 solution/教师文件或替代边界、double free 和复用语义审查。

> 验收约定：学生按 `-Stage 1/2/3` 逐步验证；原始 starter 才使用 `-ExpectIncomplete`。Demo 运行证据可导出给教师评分工具，但不能代替代码审查和解释。
