...
正式产品状态由最新 `origin/main` 与 `origin/agent-mvp` 的集成结果构成：在 P0、Lab1–Lab7、遥测可视化、自愿远程反馈和教师本地评分之上，新增通过本地 `POST /api/agent` 接入火山方舟 Agent Plan 的 AI 教学助教。它只开放六个受限工具，回答用于引导学生观察、解释、定位、预测和验证，不是标准答案、根因判定或评分依据。

数据边界分为四层：确定性诊断、预测、回放和分支比较保持本地；反馈与脱敏运行记录只在主动预览同意后发送到负责人服务；智能体问题和按需取得的受限证据可能发送到方舟，密钥仅在服务端环境变量；教师评分保持本地与人工确认。完整说明见 [AI 教学助教与数据边界](teaching-agent.md)。测试数字与在线联调状态只引用本轮最终验收报告。
...
可视化页面内的教学反馈用于评价讲解、任务、提示和运行反馈，不计算成绩。`main` 中的教师评分工具则使用来自教师指南的七套 100 分量表，支持多记录、人工检查、口试和导出。

评分工具可导入 `os-demo.run/v1`，但只建议构建/QEMU 客观状态，不执行学生代码、不读取完整日志、不自动增加分数。教学反馈可在主动同意后提交到负责人服务；教师评分页面仍为本地工具。含姓名、成绩和自由文本的导出文件必须在公开分享前人工脱敏。
...
自动测试现已覆盖协议、可视化、反馈接收与管理、评分、智能体循环、六个白名单工具、模型错误、学生端同意和纯文本渲染。具体数量、跳过项和环境限制只引用本轮最终验收报告，不沿用历史数字。
