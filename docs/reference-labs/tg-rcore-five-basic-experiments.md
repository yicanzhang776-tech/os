...
- 参考实验实现与证据采集阶段没有执行 `git push`，也没有修改比赛仓库 `main`；全部验收通过后，仅将报告、截图、日志摘要、补丁和版本信息发布到比赛仓库独立报告分支。
...
|---|---:|---:|
| 操作系统理论 | `3/5` | `4/5` |
| Rust / RISC-V 实现 | `3/5` | `4/5` |
| QEMU、测试与 AI 调试 | `1/5` | `4/5` |

实验后自评由学习者在五章实现、测试和报告证据整理完成后确认。三项分数均为主观学习反思，不替代自动测试、代码审查或比赛评审。
...
- 五个参考实验实现提交均只保留在隔离参考仓库，没有推送到官方仓库或比赛远端；比赛远端仅接收报告证据分支，不接收参考项目源码树。
