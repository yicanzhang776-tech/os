...
比较基线更新于 2026 年 8 月 16 日。本项目当前 GitLab 基线为 `main @ b4b5675`，正式交付范围包含 21 个既有远端分支：17 个教学上下文分支和 4 个辅助分支。合并前的临时文档发布分支不计入产品分支统计。GitHub 镜像状态不在本次复核范围内，不以旧镜像数字代替 GitLab 当前事实。
...
| 项目事实 | 当前值 | 证据 |
|---|---:|---|
| GitLab 正式交付分支 | 21 | 17 个教学上下文；辅助分支为 `agent-mvp`、`lab-atlas-ai-tutor`、`teacher-grading-tools`、`tg-rcore-five-lab-report`；临时文档发布分支不计入 |
...
| 本轮 Node 测试 | 586 项；580 通过、0 失败、6 跳过 | 2026-08-16 在 `main @ b4b5675` 独立执行结果 |
