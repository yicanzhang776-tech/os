...
每条评价以一行 JSON 保存在 `feedback-data/feedback.jsonl`。该目录已被 Git 忽略，不应提交到仓库。课堂异地测试时，在另一个终端仅公开 8890：
...
然后只在项目负责人电脑打开 <http://127.0.0.1:8891>。页面可按 Lab、starter/solution 和使用者身份筛选，查看五道评价题的平均值与分布、阅读文字建议，并导出 JSON、CSV 或 Markdown。8891 强制监听 `127.0.0.1`，不得放入公网隧道。
